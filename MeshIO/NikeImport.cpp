...
char* FENIKEImport::read_line(FILE* fp, char* szline, int n, bool bskiptxt)
{
...
void FENIKEImport::build_model(FENikeProject& nike)
{
...
void FENIKEImport::build_mesh(FENikeProject &nike)
{
	int i, j;
...
void FENIKEImport::build_constraints(FENikeProject& nike)
{
	...
	BC.resize(N);
	list<FENikeProject::NODAL_DISPLACEMENT>::iterator pn = nike.m_DC.begin();
	...
	{
		FENikeProject::MATERIAL& mat = nike.m_Mat[i];
		if (mat.ntype==20)
		{
			// get the constraints
...
void FENIKEImport::build_materials(FENikeProject& nike)
{
	int i;
		...
		case 18:
			{
				...
				else
				{
					...
					FEFiberMaterial* pf = pm->GetFiberMaterial();
...
void FENIKEImport::build_rigidfacets(FENikeProject& nike)
{
...
void FENIKEImport::build_discrete(FENikeProject& nike)
{
	...
	{
		FENikeProject::DISCRETE_SPRING& s = nike.m_DSP[i];
		int nmat = s.nmat - 1;
		if ((nmat >= 0) && (nmat < (int) nike.m_DMA.size()))
		{
			FENikeProject::DISCRETE_MATERIAL& m = nike.m_DMA[ nmat ];
			if (m.ntype == 1)
			{
				int n1 = s.n1 - 1;
				int n2 = s.n2 - 1;
...
void FENIKEImport::build_interfaces(FENikeProject& nike)
{
	int i, j;
...
//
bool FENIKEImport::ReadControlDeck(FENikeProject& prj)
{
...
//
bool FENIKEImport::ReadMaterialDeck(FENikeProject& prj)
{
	int i, j, l;
...
// Reads the nodes 
//
bool FENIKEImport::ReadNodes(FENikeProject& prj)
{
...
//
bool FENIKEImport::ReadBricks(FENikeProject& prj)
{
...
//
bool FENIKEImport::ReadShells(FENikeProject &prj)
{
	...
	for (i=0; i<nels; ++i)
	{
		...
		if ((s.h[0] == 0) && (s.h[1] == 0) && (s.h[2] == 0) && (s.h[3] == 0))
		{
			...
			{
				FENikeProject::MATERIAL& m = prj.m_Mat[s.nmat-1];
				s.h[0] = m.m[7][0];
				s.h[1] = m.m[7][1];
				s.h[2] = m.m[7][2];
				s.h[3] = m.m[7][3];
...
//
bool FENIKEImport::ReadRigidFacets(FENikeProject &prj)
{
...
bool FENIKEImport::ReadDiscreteElements(FENikeProject& prj)
{
...
//
bool FENIKEImport::ReadInterfaces(FENikeProject &prj)
{
	...
	for (i=0; i<ni; ++i)
	{
		FENikeProject::SLIDING_INTERFACE& si = prj.m_SI[i];
...
bool FENIKEImport::ReadLoadCurves(FENikeProject& prj)
{
	// make sure there is something to read
	FENikeProject::CONTROL& c = prj.m_Ctrl;
	int nlc = c.numlc;
...
//
bool FENIKEImport::ReadNodalForces(FENikeProject &prj)
{
	// make sure there is something to read
	FENikeProject::CONTROL& c = prj.m_Ctrl;
	int ncnf = c.numcnl;
...
bool FENIKEImport::ReadPressureFacets(FENikeProject& prj)
{
	// make sure there is something to read
	FENikeProject::CONTROL& c = prj.m_Ctrl;
	int npl = c.numpr;
...
//
bool FENIKEImport::ReadDisplacements(FENikeProject &prj)
{
	// make sure there is something to read
	FENikeProject::CONTROL& c = prj.m_Ctrl;
	int ndc = c.numdis;
...
// read  body forces
//
bool FENIKEImport::ReadBodyForce(FENikeProject &prj)
{
	char szline[MAXLINE];
	int nread, i;
...
//
bool FENIKEImport::ReadVelocities(FENikeProject &prj)
{
	// make sure there is something to read
	FENikeProject::CONTROL& c = prj.m_Ctrl;
	int nvel = c.intvel;
