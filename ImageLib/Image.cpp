...
void Image::Create(int nx, int ny, byte* pb)
{
	if (pb)
	{
		m_pb = pb;
...
void Image::StretchBlt(Image& im)
{
	byte* pd = im.m_pb;
...
byte Image::Peek(double r, double s)
{
	...
	return (byte)((h1*pb[n1] + h2*pb[n2] + h3*pb[n3] + h4*pb[n4])*0.25);
}
...
RGBImage::RGBImage(int nx, int ny)
{
	m_pb = 0;
	m_cx = m_cy = 0;
...
RGBImage::RGBImage(const RGBImage& im)
{
	m_pb = 0;
...
void RGBImage::Create(int nx, int ny)
{
	if (m_pb) delete [] m_pb;
...
RGBImage& RGBImage::operator =(const RGBImage& im)
{
	int nx = im.m_cx;
	int ny = im.m_cy;
...
bool RGBImage::SaveBMP(const char* szfile)
{
	SwapRB();
...
bool RGBImage::SaveTIF(const char* szfile)
{
...
RGBAImage::RGBAImage(int nx, int ny)
{
...
RGBAImage::RGBAImage(const RGBAImage& im)
{
	m_cx = im.m_cx;
	m_cy = im.m_cy;
...
RGBAImage& RGBAImage::operator = (const RGBAImage& im)
{
	delete [] m_pb;
...
void RGBAImage::Create(int nx, int ny)
{
	if (m_pb) delete [] m_pb;
...
void RGBAImage::StretchBlt(Image& im)
{
	int nx = Width();
	int ny = Height();
	...
	for (int y = 0; y<ny; y++)
	{
		double fy = (double) y / ny;
		for (int x = 0; x<nx; x++)
		{
			double fx = (double)x / nx;
			byte b = im.Peek(fx, fy);
			pb[0] = b;
			pb[1] = b;
			pb[2] = b;
			pb[3] = 255;
...
void RGBAImage::StretchBlt(RGBAImage& im)
{
	byte* pd = im.m_pb;
